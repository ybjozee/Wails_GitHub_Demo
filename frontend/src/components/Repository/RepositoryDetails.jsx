...
const RepositoryDetails = ({ repository, token = "" }) => {
  ...
  useEffect(() => {
    const getRepositoryDetails = async () => {
      setIsLoading(true);
      const stargazers = await GetMoreInformationFromURL(
        repository.stargazers_url,
        token
      );
      const commits = await GetMoreInformationFromURL(
        repository.commits_url.replace(/{\/[a-z]*}/, ""),
        token
      );
      const contributors = await GetMoreInformationFromURL(
